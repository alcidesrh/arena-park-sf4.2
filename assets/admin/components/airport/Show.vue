<template>
  <div>
    ...
    <router-link v-if="item" :to="{ name: 'AirportList' }" class="btn btn-default">Back to list</router-link>
...
<script>
  ...
  export default {
    computed: mapGetters({
      ...
      error: 'airport/show/error',
      loading: 'airport/show/loading',
      item: 'airport/show/item',
    }),
    methods: {
      deleteItem (item) {
        if (window.confirm('Are you sure you want to delete this item?'))
          this.$store.dispatch('airport/del/delete', item).then(response => this.$router.push({ name: 'AirportList' }));
      }
    },
    created () {
      this.$store.dispatch('airport/show/retrieve', decodeURIComponent(this.$route.params.id));
    },
    beforeDestroy() {
      this.$store.dispatch('airport/show/reset');
    }
  }
</script>
