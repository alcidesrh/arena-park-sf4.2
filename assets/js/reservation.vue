    ...
    <v-card :bind-style="{opacity: loading?0.7:1}">
      <div id="validationError">
        <v-alert
          ...
          :value="true"
          type="warning"
        >Remplissez toutes les informations requises</v-alert>
      </div>
              ...
              <v-layout row wrap>
                <v-flex xs12>
                  <v-radio-group v-model="user.sex" row>
                    <v-radio label="Mme" :value="false"></v-radio>
                    <v-radio label="Mr" :value="true"></v-radio>
                  </v-radio-group>
                </v-flex>
              </v-layout>
            </div>
                ...
                <v-flex xs12 md9>
                  ...
                  >
                    ...
                    <v-date-picker
                      ...
                      no-title
                      scrollable
                      @change="reservation.dateCarIn = reservation.dateFlyOut"
                      locale="Fr-fr"
                      :min="currentDate"
                    ></v-date-picker>
                  </v-menu>
                  ...
                  >
                    ...
                    >
                      ...
                      <v-btn
                        flat
                        color="primary"
                        @click="$refs.hourCarInDialog.save(reservation.hourCarIn)"
                      >OK</v-btn>
                    </v-time-picker>
                  </v-dialog>
                </v-flex>
                ...
                <v-flex xs12 md9>
                  ...
                  >
                    ...
                    <v-date-picker
                      ...
                      no-title
                      scrollable
                      @change="reservation.dateCarOut = reservation.dateFlyIn"
                      locale="Fr-fr"
                      :min="reservation.dateFlyOut"
                    ></v-date-picker>
                  </v-menu>
                </v-flex>
                <v-flex xs12 md3>
                  ...
                  >
                    ...
                    >
                      ...
                      <v-btn
                        flat
                        color="primary"
                        @click="$refs.hourFlyInDialog.save(reservation.hourFlyIn)"
                      >OK</v-btn>
                    </v-time-picker>
                  </v-dialog>
                </v-flex>
            ...
            <div class="section" v-if="services1.length || services2.length">
              ...
              <div v-if="services1.length">
                <v-layout row wrap>
                  ...
                  <v-flex xs4 md2>
                    <v-layout align-end justify-start row fill-height>
                      <i class="small material-icons middle_car">local_car_wash</i>
                    </v-layout>
                  </v-flex>
                  <v-flex xs4 md2>
                    <v-layout align-end justify-start row fill-height>
                      <i class="small material-icons big_car">local_car_wash</i>
                    </v-layout>
                  </v-flex>
                </v-layout>
                <v-layout row wrap v-if="services1.length>1">
                  ...
                  <v-flex xs12 md6>
                    <v-radio-group v-model="serviceRadio1" row @change="validateService(0)">
                      <v-radio :label="services1[0].prices[0].price+' CHF'" :value="0"></v-radio>
                      <v-radio :label="services1[0].prices[1].price+' CHF'" :value="1"></v-radio>
                      <v-radio :label="services1[0].prices[2].price+' CHF'" :value="2"></v-radio>
                    </v-radio-group>
                  </v-flex>
                </v-layout>
                <v-layout row wrap v-if="services1.length>2">
                  ...
                  <v-flex xs12 md6>
                    <v-radio-group v-model="serviceRadio2" row @change="validateService(1)">
                      <v-radio :label="services1[1].prices[0].price+' CHF'" :value="0"></v-radio>
                      <v-radio :label="services1[1].prices[1].price+' CHF'" :value="1"></v-radio>
                      <v-radio :label="services1[1].prices[2].price+' CHF'" :value="2"></v-radio>
                    </v-radio-group>
                  </v-flex>
                </v-layout>
                <v-layout row wrap v-if="services1.length==3">
                  ...
                  <v-flex xs12 md6>
                    <v-radio-group v-model="serviceRadio3" row @change="validateService(2)">
                      <v-radio :label="services1[2].prices[0].price+' CHF'" :value="0"></v-radio>
                      <v-radio :label="services1[2].prices[1].price+' CHF'" :value="1"></v-radio>
                      <v-radio :label="services1[2].prices[2].price+' CHF'" :value="2"></v-radio>
                    </v-radio-group>
                  </v-flex>
                </v-layout>
              </div>
...
Parking privé situé à 3 minutes de l’Aéroport de Genève
                  ...
                  <v-flex xs12 md6 v-else-if="service.id == 7">
                    <v-radio-group v-model="serviceSelected[index]" :rules="requireRules" required>
            ...
            <v-layout row wrap>
              <v-flex xs12>
                <v-checkbox :rules="requireRules" required :value="payment">
                  <div slot="label">
                    Oui, j’accepte les
                    <a href="conditions" target="_blank">Conditions Générales</a>
                    ainsi que la
                    <a
                      href="protection"
                      target="_blank"
            ...
            <v-card>
              <v-card-title primary-title>
                <div>
                  ...
                  <v-divider></v-divider>
                  <p>
                    Date et heure de décollage: {{reservation.dateFlyOut +' '+
                    reservation.hourFlyOut}}
                  </p>
                  <p>
                    Date et heure de prise en charge: {{reservation.dateCarIn +' '+
                    reservation.hourCarIn}}
                  </p>
                  <p>
                    Date et heure d’arrivée: {{reservation.dateFlyIn +' '+
                    reservation.hourFlyIn}}
                  </p>
                  <p>
                    Restitution du véhicule: {{reservation.dateCarOut +' '+
                    reservation.hourCarOut}}
                  </p>
            ...
            <v-card>
              <v-card-title primary-title>
                <div style="min-width: 100%">
                  ...
                  <div>
                    Prise en charge:
                    <span class="right">{{tarif.priceCharge}} CHF</span>
                  </div>
                  <div>
    ...
    <v-layout justify-center aling-center class="loading-div" v-if="loading">
      <v-progress-linear style="max-width: 300px" :indeterminate="true"></v-progress-linear>
    </v-layout>
  </v-app>
</template>
<script>
...
export default {
  data: () => ({
    ...
    success: false,
    errorMessage: false,
    requireRules: [v => !!v || "Ce champ est obligatoire"],
    emailRules: [
  ...
  methods: {
    ...
    validateFields() {
      if (this.$refs.form.validate()) {
        ...
        if (this.serviceRadio3 || this.serviceRadio3 === 0) {
          services.push({
            id: this.services1[2].id,
            price: this.serviceRadio3
          });
        } else {
          if (this.serviceRadio1 || this.serviceRadio1 === 0) {
            services.push({
              id: this.services1[0].id,
              price: this.serviceRadio1
            });
          }
          if (this.serviceRadio2 || this.serviceRadio2 === 0) {
            services.push({
              id: this.services1[1].id,
              price: this.serviceRadio2
            });
          }
        }
        if (this.serviceSelected.length) {
          for (let i = 0, service; i < this.serviceSelected.length; i++) {
            service = this.services2.filter(
              item => item.id == this.serviceSelected[i]
            );
            if (service.length) services.push({ id: service[0].id, price: 0 });
          }
        }
    ...
    searchUser(val) {
      if (
        ...
        axios
          .get(this.getEndPoint("/users?email=" + this.user.email))
          .then(function(response) {
    ...
    calculePrice() {
      let days =
        moment(this.reservation.dateCarOut).diff(
          moment(this.reservation.dateCarIn),
          "days"
        ) + 1;
      ...
      if (typeof this.reservation.services != typeof undefined) {
        for (let i = 0, service; i < this.reservation.services.length; i++) {
          service = this.services.filter(
            item => item.id == this.reservation.services[i].id
          );
          this.reservation.total =
            this.reservation.total +
            parseFloat(
              service[0].prices[this.reservation.services[i].price].price
            );
        }
      }
    ...
    getAirportName() {
      let airport = this.airports.filter(
        item => item.id == this.reservation.airport
      );
  ...
  created() {
    ...
    if (id)
      axios
        .get(this.getEndPoint("/users?id=" + id))
        .then(function(response) {
          $this.loading = false;
          if (
            response.data["hydra:member"].length &&
            ...
            $this.setUser(response.data["hydra:member"][0]);
          }
        })
        .catch(function(error) {
          $this.loading = false;
    ...
    axios
      ...
      .then(function(response) {
        ...
        $this.services1 = $this.services.filter(
          item => item.prices.length == 3
        );
        $this.services2 = $this.services.filter(
