...
class GenerateContract
{
    ...
    public function generateContract( EntityManagerInterface $entityManager, User $user = null, Car $car = null, Reservation $reservation = null, Tarif $tarif, $onlinePaid = false )
    {
...
//            $document = $this->loadTemplate('doc/contract-enterprise.docx');
        ...
        $document->setValue( 'pCharge', $tarif->getPriceCharge() );
        $document->setValue( 'annulation', $tarif->getAnnulation() );
...
//        $document->setValue( 'baggage', $reservation[ 'baggage' ] );
        ...
        if ( $reservation->getPaymentType() === 1 ) {
            ...
            $document->setValue( 'paymode', utf8_decode( "Payé en ligne par " ) . ucwords( $reservation->getPaymentType() == 2?'Visa':'MasterCard' ) );
        }
        ...
        if ( $services = $reservation->getServices() ) {
            foreach ( $services as $value ) {
                ...
                $document->setValue( 'servicec' . $cont++, $service->getPrices()[$value['price']]['price'] . ',00' );
...
//                $chargeService += $value[ 'charge' ];
        ...
        $document->setValue( 'charge', intval(($dDiff->days + 1) * $tarif->getDay()) . ',00' );

        $total = $reservation->getPayment();// + $percent;
        if($tarif->getActiveDescount()){
            $descount = $tarif->getDescount();
            $document->setValue( 'total', intval($total * (1 + $descount/100)).',00' );
            $document->setValue('percent', "-$descount%");
            $document->setValue( 'totalp', $total.',00' );
        }
        else{
            $document->setValue( 'totalp', $total.',00' );
        }
        ...
        $contractPath = self::PATH.'/contracts/user' . $user->getId() . 'date' . $reservation->getCreateAt()->format('Ymdhms');
